...
export async function createMainModule(
  data: mainModules,
): Promise<mainModulesDocument> {
  try {
    ...
    throw new Error(
      `Could not create new Main Module: ${(err as Error).message}`,
    );
  }
}
...
export async function getAllMainModules(): Promise<mainModulesDocument[]> {
  try {
    const modules = await MainModule.find().sort({ order: 1 }).lean().exec();
